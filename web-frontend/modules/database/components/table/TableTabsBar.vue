    ...
    <div ref="scrollContainer" class="table-tabs-bar__scroll-container">
      <div class="table-tabs-bar__tabs">
        ...
        >
          ...
          <i v-else-if="!isTableReadOnly(table) && ['distribution management', 'catalog pipeline', 'catalogue pipeline', 'distribution pipeline', 'production pipeline', 'production catalogue'].includes((database.name || '').trim().toLowerCase())" class="table-tabs-bar__tab-icon iconoir-edit-pencil" style="margin-right: 4px; font-size: 14px; opacity: 0.7;"></i>
...
export default {
  ...
  computed: {
    ...
    canCreateTable() {
      ...
      if (dbName === 'live catalogue' || ['distribution management', 'catalog pipeline', 'catalogue pipeline', 'distribution pipeline', 'production pipeline', 'production catalogue'].includes(dbName)) {
        return false
      }
    ...
    isTableReadOnly() {
      return (table) => {
        ...
        if (['catalog pipeline', 'catalogue pipeline', 'distribution pipeline', 'production pipeline', 'production catalogue'].includes(dbName)) {
